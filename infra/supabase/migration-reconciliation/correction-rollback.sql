-- Restore the exact reviewed function and owner-only privilege state.
-- Use only under separate recovery authorization after a clean canonical
-- preflight proves the target signature is absent.
BEGIN;
...
BEGIN
  IF current_database() <> 'postgres'
    ...
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'ROLLBACK_TARGET_REFUSED';
  END IF;
  ...
  IF to_regclass('public._prisma_migrations') IS NULL
    ...
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'ROLLBACK_LEDGER_REFUSED';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'ROLLBACK_CONFLICT_REFUSED';
  END IF;
END
$guard$;
...
ALTER FUNCTION public.pathways_prevent_audit_mutation() OWNER TO prisma;
REVOKE ALL PRIVILEGES ON FUNCTION public.pathways_prevent_audit_mutation() FROM PUBLIC;
REVOKE ALL PRIVILEGES ON FUNCTION public.pathways_prevent_audit_mutation()
  FROM anon, authenticated, service_role, pathways_runtime;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'ROLLBACK_POSTFLIGHT_REFUSED';
  END IF;
END
$postflight$;

COMMIT;
SELECT 'PATHWAYS_AUDIT_FUNCTION_ROLLBACK_COMMITTED';
