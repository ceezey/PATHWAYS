...
export const metricCellSchema = z.object({
  ...
  if (cell.state === 'ZERO' && cell.value !== '0') ctx.addIssue({ code: 'custom', message: 'ZERO requires an exact zero.' })
...
export const indicatorBindingSchema = z.object({
  ...
  const formMetric = binding.recipe === 'FORM_NUMERIC_SUM' || binding.recipe === 'FORM_NUMERIC_AVERAGE'
  if (formMetric && (!binding.formId || !binding.fieldId || !binding.formVersion || binding.activityId)) {
    ctx.addIssue({ code: 'custom', message: 'Form metrics require an exact form ID/version and numeric field ID only.' })
  }
  ...
  if (binding.recipe === 'ACTIVITY_COMPLETION_PERCENTAGE' && binding.activityId) {
    ctx.addIssue({ code: 'custom', message: 'Activity completion uses the project activity population, not one activity.' })
  }
})
...
export const createIndicatorSchema = z.object(definitionFields).strict().superRefine((input, ctx) => {
  try {
    validateMetricPeriod(input.periodStart, input.periodEnd)
    for (const value of [input.baseline, input.target]) if (value !== null) normalizeMetricDecimal(value, input.numericKind)
    if (input.numericKind === 'COUNT' && input.displayPrecision !== 0) throw new Error('Counts use zero display decimal places.')
    if ((input.mode === 'DERIVED') !== Boolean(input.binding)) throw new Error('Exactly derived indicators require a binding.')
    const recipe = input.binding?.recipe
    if (recipe && ['PARTICIPATION_RECORD_COUNT', 'DISTINCT_ATTENDING_INDIVIDUALS', 'EFFECTIVE_JOURNEY_EVENT_COUNT'].includes(recipe) && input.numericKind !== 'COUNT') throw new Error('This recipe produces a count.')
    if (recipe === 'ATTENDANCE_RECORDS_PER_INDIVIDUAL' && input.numericKind !== 'RATIO') throw new Error('This recipe produces an uncapped non-negative ratio.')
    if (recipe === 'ACTIVITY_COMPLETION_PERCENTAGE' && input.numericKind !== 'PERCENTAGE') throw new Error('This recipe produces a percentage.')
    if (input.baseline !== null && input.target !== null && input.direction !== 'DESCRIPTIVE') {
      const progress = indicatorProgress({ state: 'AVAILABLE', value: '1', reason: null }, input.baseline, input.target, input.direction)
      if (progress.reason === 'DIRECTION_CONFLICT') throw new Error('Target conflicts with the selected direction.')
    }
  } catch (error) { ctx.addIssue({ code: 'custom', message: error instanceof Error ? error.message : 'Invalid metric definition.' }) }
})
...
export const archiveIndicatorSchema = z.object({ expectedRevision: z.number().int().min(1) }).strict()
export const manualMeasurementSchema = z.object({
  ...
  try { validateMetricPeriod(input.periodStart, input.periodEnd) } catch { ctx.addIssue({ code: 'custom', message: 'Invalid measurement period.' }) }
  if (Boolean(input.correctsMeasurementId) !== Boolean(input.correctionReason)) ctx.addIssue({ code: 'custom', message: 'A correction requires the previous measurement ID and a reason.' })
})
export type ManualMeasurementInput = z.infer<typeof manualMeasurementSchema>

export const monitoringIndicatorSchema = z.object({
  id: uuid,
  projectId: uuid,
...
export const dashboardQuerySchema = z.object({
  ...
  if (input.projectId && input.programId) ctx.addIssue({ code: 'custom', message: 'Choose a project or a program, not both.' })
  if (Boolean(input.periodStart) !== Boolean(input.periodEnd)) ctx.addIssue({ code: 'custom', message: 'Both period dates are required.' })
  if (input.periodStart && input.periodEnd) {
    try { validateMetricPeriod(input.periodStart, input.periodEnd) } catch (error) { ctx.addIssue({ code: 'custom', message: error instanceof Error ? error.message : 'Invalid period.' }) }
  }
})
export type DashboardQuery = z.infer<typeof dashboardQuerySchema>
const bucket = z.object({ key: z.string().max(80), label: z.string().max(160), metric: metricCellSchema }).strict()
const contextFields = {
  contractVersion: z.literal(P06_CONTRACT_VERSION),
  periodStart: date,
  periodEnd: date,
...
export const sadddDashboardSchema = z.object({
  ...contextFields,
...
export const monitoringDashboardSchema = z.object({
  ...contextFields,
  projects: z.array(z.object({ id: uuid, code: z.string(), title: z.string() }).strict()).max(100),
...
export function formatMetricCell(cell: MetricCell): string {
  ...
  return cell.state === 'SUPPRESSED' ? 'Suppressed' : cell.state === 'NOT_APPLICABLE' ? 'Not applicable' : 'Not available'
}
