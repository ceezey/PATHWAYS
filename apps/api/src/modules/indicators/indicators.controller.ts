import { Body, Controller, ForbiddenException, Get, Header, Inject, Param, Patch, Post, Req } from '@nestjs/common'
...
export class IndicatorsController {
  ...
  @Header('Cache-Control', 'private, no-store')
  @RequirePermission('monitoring.read')
  get(@Req() request: AuthenticatedRequest, @Param('projectId') projectId: string, @Param('indicatorId') indicatorId: string) {
  ...
  update(@Req() request: AuthenticatedRequest, @Param('projectId') projectId: string, @Param('indicatorId') indicatorId: string, @Body() body: unknown) {
    return this.service.update(identity(request), projectId, indicatorId, body)
  }
  @Post(':indicatorId/measurements')
  @Header('Cache-Control', 'private, no-store')
  @RequirePermission('indicators.update')
  measure(@Req() request: AuthenticatedRequest, @Param('projectId') projectId: string, @Param('indicatorId') indicatorId: string, @Body() body: unknown) {
    return this.service.measure(identity(request), projectId, indicatorId, body)
  }
  @Post(':indicatorId/archive')
  @Header('Cache-Control', 'private, no-store')
  @RequirePermission('indicators.update')
  archive(@Req() request: AuthenticatedRequest, @Param('projectId') projectId: string, @Param('indicatorId') indicatorId: string, @Body() body: unknown) {
