...
import { createIndicatorSchema, manualMeasurementSchema, parseIndicatorInput, updateIndicatorSchema } from './indicators.dto'
...
export function monitoringSqlError(error: unknown): never {
  ...
  if (code === 'P2002' || code === 'P2034' || ['23505', '23514', '40001', '40P01'].includes(String(sqlCode))) {
    throw new ConflictException('The indicator or measurement changed, conflicts, or violates its contract. Reload before retrying.')
  }
  ...
  if (code === 'P2028' || ['57014', '42P01', '42703', '42883', '22023'].includes(String(sqlCode))) {
    throw new ServiceUnavailableException('Monitoring verification is unavailable or exceeded its query limit.')
  }
  throw error
}
...
export class IndicatorsService {
  ...
  private async requireProject(tx: Tx, actor: ApplicationIdentity, projectId: string) {
    ...
    const project = await tx.project.findFirst({ where: { AND: [projectScope(actor), { id: projectId.toLowerCase() }] }, select: { id: true } })
  ...
  async readInTransaction(tx: Tx, actor: ApplicationIdentity, projectIds: string[], options: { indicatorId?: string; periodStart?: string; periodEnd?: string } = {}): Promise<MonitoringIndicator[]> {
    if (projectIds.length === 0) return []
    if (projectIds.length > 100 || projectIds.some((id) => !UUID_PATTERN.test(id))) throw new BadRequestException('Narrow the monitoring project scope.')
    ...
    try {
      ...
      `)
    } catch (error) { monitoringSqlError(error) }
    if (rows.length > 100) throw new BadRequestException('More than 100 indicator definitions match; narrow the project scope.')
    return rows.map((row) => {
      const parsed = monitoringIndicatorSchema.safeParse({
        ...row, progress: missingMetric('NOT_YET_CALCULATED'), contractVersion: P06_CONTRACT_VERSION,
      })
      if (!parsed.success) throw new ServiceUnavailableException('Stored indicator contract is unavailable.')
  ...
  private async readOne(tx: Tx, actor: ApplicationIdentity, projectId: string, indicatorId: string) {
  ...
  list(identity: ApplicationIdentity, projectId: string) {
    return withAuthorizedOperation(this.prisma, identity, 'monitoring.read', async (tx, actor) => this.readInTransaction(tx, actor, [await this.requireProject(tx, actor, projectId)]))
  }
  get(identity: ApplicationIdentity, projectId: string, indicatorId: string) {
    return withAuthorizedOperation(this.prisma, identity, 'monitoring.read', async (tx, actor) => this.readOne(tx, actor, await this.requireProject(tx, actor, projectId), this.indicatorId(indicatorId)))
  }
  private audit(tx: Tx, actor: ApplicationIdentity, projectId: string, action: string, entityId: string, changes: Prisma.InputJsonObject) {
    return tx.auditLog.create({ data: { organizationId: actor.organizationId, actorUserId: actor.userId, projectId, action, entityType: 'ProjectIndicator', entityId, changes } })
  }
  private async validateBinding(tx: Tx, actor: ApplicationIdentity, projectId: string, input: CreateIndicatorInput) {
    const binding = input.binding
    if (!binding) return
    if (binding.activityId) {
      const activity = await tx.projectActivity.findFirst({ where: { id: binding.activityId, organizationId: actor.organizationId, projectId, archivedAt: null, status: { not: 'CANCELLED' } }, select: { id: true } })
      if (!activity) throw new NotFoundException('Bound activity unavailable.')
    }
    if (binding.formId) {
      const form = await tx.digitalForm.findFirst({ where: { id: binding.formId, organizationId: actor.organizationId, projectId, version: binding.formVersion, status: 'PUBLISHED', archivedAt: null }, select: { id: true } })
      if (!form) throw new NotFoundException('Published form version unavailable.')
      const field = await tx.formField.findFirst({ where: { id: binding.fieldId, organizationId: actor.organizationId, projectId, formId: form.id, dataType: { in: ['INTEGER', 'DECIMAL'] } }, select: { id: true } })
  ...
  async create(identity: ApplicationIdentity, projectId: string, value: unknown) {
    ...
    return withAuthorizedOperation(this.prisma, identity, 'indicators.create', async (tx, actor) => {
      ...
      const [size] = await tx.$queryRaw<Array<{ count: bigint }>>`SELECT count(*) FROM pathways.project_indicators WHERE organization_id=${actor.organizationId}::uuid AND project_id=${id}::uuid AND archived_at IS NULL`
      if (size && size.count >= 100n) throw new ConflictException('Archive unused indicators before adding more than 100 active definitions.')
      const indicatorId = randomUUID()
      const baseline = input.baseline === null ? null : normalizeMetricDecimal(input.baseline, input.numericKind)
      const target = input.target === null ? null : normalizeMetricDecimal(input.target, input.numericKind)
      const unit = input.numericKind === 'COUNT' ? 'COUNT' : input.numericKind === 'PERCENTAGE' ? 'PERCENTAGE' : 'OTHER'
      try {
        await tx.$executeRaw`
        ...
        await this.audit(tx, actor, id, 'PROJECT_INDICATOR_CREATED', indicatorId, { mode: input.mode, numericKind: input.numericKind, recipe: input.binding?.recipe ?? null, contractVersion: P06_CONTRACT_VERSION })
      } catch (error) { monitoringSqlError(error) }
      return this.readOne(tx, actor, id, indicatorId)
    })
  }
  async update(identity: ApplicationIdentity, projectId: string, indicatorId: string, value: unknown) {
    const input: UpdateIndicatorInput = parseIndicatorInput(updateIndicatorSchema, value)
    return withAuthorizedOperation(this.prisma, identity, 'indicators.update', async (tx, actor) => {
      const id = await this.requireProject(tx, actor, projectId)
      const selected = this.indicatorId(indicatorId)
      try {
        const count = await tx.$executeRaw`UPDATE pathways.project_indicators SET name=${input.name},description=${input.description ?? null},revision=revision+1,updated_at=CURRENT_TIMESTAMP WHERE organization_id=${actor.organizationId}::uuid AND project_id=${id}::uuid AND id=${selected}::uuid AND revision=${input.expectedRevision} AND archived_at IS NULL`
        if (count !== 1) throw new ConflictException('Indicator changed or is unavailable. Reload before editing.')
        await this.audit(tx, actor, id, 'PROJECT_INDICATOR_LABEL_UPDATED', selected, { revision: input.expectedRevision + 1 })
      } catch (error) { monitoringSqlError(error) }
      return this.readOne(tx, actor, id, selected)
    })
  }
  archive(identity: ApplicationIdentity, projectId: string, indicatorId: string, expectedRevision: number) {
    return withAuthorizedOperation(this.prisma, identity, 'indicators.update', async (tx, actor) => {
      const id = await this.requireProject(tx, actor, projectId)
      const selected = this.indicatorId(indicatorId)
      try {
        const count = await tx.$executeRaw`UPDATE pathways.project_indicators SET archived_at=CURRENT_TIMESTAMP,revision=revision+1,updated_at=CURRENT_TIMESTAMP WHERE organization_id=${actor.organizationId}::uuid AND project_id=${id}::uuid AND id=${selected}::uuid AND revision=${expectedRevision} AND archived_at IS NULL`
        if (count !== 1) throw new ConflictException('Indicator changed or is unavailable. Reload before archiving.')
        await this.audit(tx, actor, id, 'PROJECT_INDICATOR_ARCHIVED', selected, { revision: expectedRevision + 1 })
      } catch (error) { monitoringSqlError(error) }
      return this.readOne(tx, actor, id, selected)
    })
  }
  async measure(identity: ApplicationIdentity, projectId: string, indicatorId: string, value: unknown) {
    ...
    return withAuthorizedOperation(this.prisma, identity, 'indicators.update', async (tx, actor) => {
      ...
      if (definition.mode !== 'MANUAL' || !definition.numericKind) throw new ConflictException('Manual measurements require a reviewed manual definition.')
      if (definition.periodStart !== input.periodStart || definition.periodEnd !== input.periodEnd) throw new ConflictException('The measurement must use its definition reporting period.')
      let exact: string
      try { exact = normalizeMetricDecimal(input.value, definition.numericKind) } catch { throw new BadRequestException('Measurement does not match the indicator numeric domain.') }
      const requestHash = createHash('sha256').update(JSON.stringify({ projectId: id, indicatorId: selected, periodStart: input.periodStart, periodEnd: input.periodEnd, value: exact, source: input.source, note: input.note ?? null, correctsMeasurementId: input.correctsMeasurementId ?? null, correctionReason: input.correctionReason ?? null })).digest('hex')
      const [retry] = await tx.$queryRaw<Array<{ hash: string }>>`SELECT request_hash AS hash FROM pathways.project_indicator_measurements WHERE organization_id=${actor.organizationId}::uuid AND recorded_by_id=${actor.userId}::uuid AND client_measurement_id=${input.clientMeasurementId}::uuid`
      if (retry) {
        if (retry.hash !== requestHash) throw new ConflictException('The idempotency key was already used for different input.')
        return definition
      }
      if (definition.status === 'ARCHIVED') throw new ConflictException('Archived indicators do not accept measurements.')
      if (definition.measurementId !== (input.correctsMeasurementId ?? null)) throw new ConflictException('Correct the latest measurement with a reason, or reload before creating the first value.')
      ...
      try {
        ...
        `
        await this.audit(tx, actor, id, input.correctsMeasurementId ? 'INDICATOR_MEASUREMENT_CORRECTED' : 'INDICATOR_MEASUREMENT_RECORDED', selected, { measurementId, correctsMeasurementId: input.correctsMeasurementId ?? null })
      } catch (error) { monitoringSqlError(error) }
      return this.readOne(tx, actor, id, selected)
    })
  }
