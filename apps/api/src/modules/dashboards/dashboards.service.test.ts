...
const actor: ApplicationIdentity = {
  ...
  aal: 'aal2',
  organizationId,
  userId: '78000000-0000-4000-8000-000000000004',
  fullName: 'Synthetic executive',
  roles: ['PROGRAM_MANAGER'],
  permissions: ['analytics.read', 'monitoring.read', 'beneficiaries.aggregates.read'],
...
const monitoringBody = {
  activities: [],
  milestones: [],
  participationRecords: zero,
  attendingIndividuals: zero,
  enrolledBeneficiaryRecords: zero,
  enrolledIndividuals: zero,
}
const sadddBody = {
  total: suppressed,
  sex: [],
  age: [
    { key: '0-9', label: '0-9', metric: suppressed },
    { key: 'Unknown', label: 'Unknown', metric: suppressed },
  ],
  disability: [],
  completeness: [],
}
...
describe('P06 DashboardsService aggregate-only boundary', () => {
  ...
  beforeEach(() => {
    ...
    tx.project.findMany.mockResolvedValue([
      { id: projectId, code: 'P06_A', title: 'Synthetic authorized project' },
    ])
  ...
  it.each([
  ...
  ] as const)('keeps %s inside server-derived project scope', async (role) => {
    ...
    expect(tx.project.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        take: 101,
        where: { AND: [expect.objectContaining({ organizationId })] },
      }),
    )
  ...
  it('refuses extra response fields rather than redacting fetched Beneficiary rows', async () => {
    tx.$queryRaw.mockImplementation(async (query: unknown) =>
      sqlText(query).includes('p06_saddd')
        ? [{ data: { ...sadddBody, beneficiaryIds: ['must-not-escape'] } }]
        : [],
    )
  ...
  it('rejects unapproved cross-filters and over-broad queries before aggregation', async () => {
    await expect(service.saddd(actor, { sex: 'FEMALE' })).rejects.toBeInstanceOf(
      BadRequestException,
    )
    await expect(
      service.monitoring(actor, { periodStart: '2026-01-01', periodEnd: '2027-01-02' }),
    ).rejects.toBeInstanceOf(BadRequestException)
    tx.project.findMany.mockResolvedValue(
      Array.from({ length: 101 }, () => ({ id: projectId, code: 'P', title: 'Synthetic' })),
    )
  ...
  it('does not distinguish guessed foreign IDs from unavailable project scope', async () => {
    tx.project.findMany.mockResolvedValue([])
    await expect(
      service.monitoring(actor, { projectId: '78000000-0000-4000-8000-000000000099' }),
  ...
  it('re-queries each request and carries the current organization into the SQL parameters', async () => {
    ...
    await service.monitoring(
      { ...actor, organizationId: '78000000-0000-4000-8000-000000000098' },
      {},
    )
    const calls = tx.$queryRaw.mock.calls
      .map((call) => call[0])
      .filter((query) => sqlText(query).includes('p06_monitoring'))
  ...
  it('uses a finite database statement deadline and reports timeout as unavailable', async () => {
    tx.$queryRaw.mockImplementation(async (query: unknown) => {
      if (sqlText(query).includes('p06_monitoring'))
    ...
    expect(
      tx.$queryRaw.mock.calls.some((call) =>
        sqlText(call[0]).includes("statement_timeout','3000'"),
      ),
    ).toBe(true)
  })
})
