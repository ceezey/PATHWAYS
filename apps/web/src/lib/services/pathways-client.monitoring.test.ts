...
vi.mock('@/lib/supabase/client', () => ({ getBrowserSupabaseClient: () => ({ auth: { getSession: browser.getSession } }) }))
...
const context = { contractVersion: 'p06.v1', periodStart: '2026-06-01', periodEnd: '2026-06-30', businessTimeZone: 'Asia/Manila', generatedAt: '2026-09-15T00:00:00Z', refresh: 'READ_TIME_NO_CACHE' }
const missing = { state: 'MISSING', value: null, reason: 'RELEASE_POLICY_REVIEW_REQUIRED' }
const dashboard = { ...context, projects: [{ id: projectId, code: 'P06_A', title: 'Synthetic project' }], scopeProjectCount: 1, activities: [], milestones: [], indicators: [], indicatorNote: 'No indicator matches the exact period.', participationRecords: missing, attendingIndividuals: missing, enrolledBeneficiaryRecords: missing, enrolledIndividuals: missing }
const saddd = { ...context, population: 'DISTINCT_INDIVIDUALS_WITH_OVERLAPPING_ENROLLMENT', demographicBasis: 'CURRENT_PROFILE_NOT_HISTORICAL_SNAPSHOT', privacy: { threshold: 5, complementarySuppression: true, policy: 'G4_CALCULATION_RELEASE_PENDING', crossFilters: 'UNAVAILABLE_PENDING_POLICY' }, total: missing, sex: [], age: [], disability: [], completeness: [] }
const fetcher = vi.fn()
...
describe('P06 client calls actual authorized APIs', () => {
  beforeEach(() => {
    ...
    vi.stubGlobal('document', { cookie: `pathways-context=${encodeURIComponent(JSON.stringify({ authUserId: id, organizationId, userId }))}` })
    browser.getSession.mockResolvedValue({ data: { session: { access_token: 'synthetic-token', user: { id } } }, error: null })
  ...
  it('uses a current token and no-store monitoring request with explicit period filters', async () => {
    ...
    const result = await pathwaysClient.getMonitoringDashboard({ projectId, periodStart: '2026-06-01', periodEnd: '2026-06-30' })
    expect(result.attendingIndividuals.value).toBeNull()
    expect(fetcher).toHaveBeenCalledWith(expect.stringContaining(`/dashboards/monitoring?projectId=${projectId}`), expect.objectContaining({ cache: 'no-store', credentials: 'omit', headers: expect.objectContaining({ Authorization: 'Bearer synthetic-token' }) }))
  ...
  it('uses aggregate-only SADDD and never asks for raw Beneficiary records', async () => {
    ...
    expect(fetcher.mock.calls.every((call) => String(call[0]).includes('/dashboards/saddd'))).toBe(true)
  ...
  it('does not use an empty array or zero fallback after denied indicator access', async () => {
    ...
    await expect(pathwaysClient.getProjectIndicators(projectId)).rejects.toMatchObject({ code: 'forbidden' })
  })
  ...
  it('fails closed when the current session disappears', async () => {
    ...
    await expect(pathwaysClient.getMonitoringDashboard()).rejects.toMatchObject({ code: 'unauthorized' })
    expect(fetcher).not.toHaveBeenCalled()
  })
