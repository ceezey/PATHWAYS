...
export const BeneficiaryAccessGate = ({ onVerified }: { onVerified: () => void }) => {
  ...
  const verify = async () => {
    ...
    if (!response.ok || !result.ok) {
      ...
      setMessage(result.message ?? 'The PIN could not be verified.')
      setPin('')
      return
    }
      ...
      <Dialog open onOpenChange={(open) => !open && router.push('/dashboard')}>
        ...
        >
          <div className="space-y-5">
            <div className="rounded-lg border border-warning/20 bg-warning/10 p-4 text-sm leading-6 text-warning">
