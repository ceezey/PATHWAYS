...
const data = vi.hoisted(() => ({
  monitoring: {
    ...
    participationRecords: {
      state: 'MISSING',
      value: null,
      reason: 'RELEASE_POLICY_REVIEW_REQUIRED',
    },
    ...
    milestones: [],
    indicators: [],
    indicatorNote: 'No indicator definition for the selected period.',
  },
}))
vi.mock('@/hooks/use-current-role', () => ({
  useCurrentRole: () => ({ profile: { permissions: ['analytics.read'] } }),
}))
vi.mock('@/hooks/use-display-labels', () => ({
  useDisplayLabels: () => ({ labels: { moduleAnalytics: 'Monitoring and analytics' } }),
}))
vi.mock('@/lib/services/pathways-client', () => ({ pathwaysClient: {} }))
vi.mock('./use-monitoring-read', () => ({
  useMonitoringRead: (key: string) => ({
    data:
      key === 'monitoring-project-options' ? [] : key.startsWith('saddd:') ? null : data.monitoring,
    loading: false,
    error: null,
    reload: () => undefined,
  }),
...
describe('P06 analytics interface uses aggregate contracts', () => {
  ...
  it('keeps demographic intersections and location drill-through unavailable', () => {
    ...
    expect(html).toContain(
      'Demographic intersections, location and activity drill-through are not enabled.',
    )
