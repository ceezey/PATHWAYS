...
describe('P06 server-produced chart values', () => {
  const buckets = [
    { key: '0-9', label: '0-9', metric: numericMetric('5') },
    {
      key: 'Unknown',
      label: 'Unknown',
      metric: { state: 'SUPPRESSED' as const, value: null, reason: 'COMPLEMENTARY_SUPPRESSION' },
    },
    {
      key: 'custom',
      label: 'Server-provided category',
      metric: { state: 'MISSING' as const, value: null, reason: 'RELEASE_POLICY_REVIEW_REQUIRED' },
    },
  ...
  it('uses API category labels, including 0-9 and Unknown, without rebuilding age bands', () => {
    expect(aggregateChartOption(buckets, 'Age').xAxis.data).toEqual([
      '0-9',
      'Unknown',
  ...
  it('renders a truthful table, not a zero-valued chart, when no values may be released', () => {
    const html = renderToStaticMarkup(
      createElement(SadddChart, { label: 'Age', buckets: buckets.slice(1, 3) }),
    )
