...
import { getProofFilePreview } from '@/lib/demo-state/proof-file-previews'
...
const fileSize = (size: number) => {
  if (!size) return 'Seeded file metadata'
        ...
        >
          {selected && preview ? (
            ...
            <p className="rounded-sm border border-info/25 bg-info-subtle p-4 text-sm text-info">
              This seeded file, or a file from an earlier browser session, has metadata only. Submit
              the file again in the current frontend session to preview its contents.
            </p>
          )}
        </DialogShell>
      </Dialog>
