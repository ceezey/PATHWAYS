...
import { reviewExpense } from '@/lib/demo-state/projects'
...
export function ActivityExpenseReviewDialog({
  ...
  const submit = (verified: boolean) => {
    ...
    setError('')
    try {
      reviewExpense(expense.id, verified, reason)
      toast.success(verified ? 'Expense validated and added to the ledger.' : 'Expense returned.')
      onOpenChange(false)
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : 'The expense review could not be saved.')
    } finally {
      setSubmitting(false)
    }
  }
