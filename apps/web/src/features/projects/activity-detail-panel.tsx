...
import { requestActivityExtension } from '@/lib/demo-state/projects'
import type { DemoExpense } from '@/lib/demo-state/store'
...
import { ActivityExpenseReviewDialog } from './activity-expense-review-dialog'
...
const proofVersion = (activity: Activity, proof: ActivityProof) =>
  proof.version ?? activity.submittedProof.indexOf(proof) + 1

const proofTone = (status: ActivityProof['status']) => {
  if (status === 'Approved' || status === 'Accepted' || status === 'Validated') return 'success'
  if (status === 'Returned' || status === 'Flagged') return 'danger'
...
export const ActivityDetailContent = ({
  ...
  const [expenseReviewTarget, setExpenseReviewTarget] = useState<DemoExpense | null>(null)
  ...
  const correctionRequired = latestProof?.status === 'Returned' || latestProof?.status === 'Flagged'
  const pendingExpenses = state.expenses.filter(
    (expense) => expense.activityId === activity.id && expense.status === 'For Verification',
  )

  useEffect(() => {
    if (!requestedExpenseId) return
    const requested = pendingExpenses.find((expense) => expense.id === requestedExpenseId)
    if (requested) setExpenseReviewTarget(requested)
  }, [requestedExpenseId, pendingExpenses])
    ...
    <div className="space-y-5 pb-1">
                ...
                >
                  ...
                  {proof.returnReason ? (
                    <p className="mt-3 rounded-sm border border-danger/25 bg-danger-subtle p-3 text-sm text-danger">
                      Return reason: {proof.returnReason}
                    </p>
                  ) : null}
                  ...
                  {isLatest && canDecideProof && proof.status === 'Validated' ? (
                    <Button
                      className="mt-4 gap-2"
      ...
      <div className="sticky bottom-0 -mx-1 grid grid-cols-1 gap-2 border-t border-border bg-card/95 px-1 pb-1 pt-4 backdrop-blur">
        ...
        {canRequestExtension && activity.status !== 'Completed' ? (
          <Button
            className="gap-2"
            disabled={Boolean(activity.extensionRequestedAt)}
            onClick={() => {
              const updated = requestActivityExtension(activity.id)
          ...
          >
            ...
            {activity.extensionRequestedAt ? 'Extension requested' : 'Request an extension'}
          </Button>
        ) : null}
      </div>
