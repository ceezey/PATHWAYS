...
const usePublicProjection = (fallback: PublicProjectRecord[], unavailable = false) => {
  return {
    projects: fallback,
...
export const PublicHomeDashboard = ({
  projects: fallback,
  unavailable = false,
}: { projects: PublicProjectRecord[]; unavailable?: boolean }) => {
  const { projects, maintenance } = usePublicProjection(fallback, unavailable)
        ...
        <CardContent className="space-y-5">
          ...
          <div className="grid gap-3 text-sm sm:grid-cols-2">
            ...
            <Metric
              label="Progress"
              value={
                project.progressTrend.at(-1) === undefined
                  ? 'Not available'
